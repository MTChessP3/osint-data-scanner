...
function drawProfessionalCover(
  ...
  setFont(doc, true).fillColor(C.coverAccent).fontSize(18)
    .text('◆', M.left + 9, logoY + 8, { width: 22 });
  setFont(doc, true).fillColor(C.coverLight).fontSize(10)
    .text('OSINT DATA SCANNER', M.left + 50, logoY + 12, { width: 200 });
  ...
  setFont(doc, true).fillColor('#fca5a5').fontSize(8)
    .text('⚠ CLASIFICACIÓN: CONFIDENCIAL', confX + 6, logoY + 7, { width: 118 });

  // Main title area
  doc.y = 160;
  setFont(doc, true).fillColor(C.coverLight).fontSize(28)
    .text('INFORME DE', M.left, doc.y, { width: CW, align: 'center' });
  doc.moveDown(0.2);
  setFont(doc, true).fillColor(C.coverAccent).fontSize(32)
    .text('INVESTIGACIÓN', M.left, doc.y, { width: CW, align: 'center' });
  doc.moveDown(1);
  ...
  if (opts.subtitle) {
    setFont(doc, false).fillColor(C.coverSlate).fontSize(12)
      .text(opts.subtitle, M.left, doc.y, { width: CW, align: 'center' });
    doc.moveDown(0.8);
  }
  ...
  // Inner filled arc
  const scoreAngle = (opts.riskScore / 100) * 2 * Math.PI;
  const startAngle = -Math.PI / 2;
  doc.strokeColor(opts.riskColor).lineWidth(3);
  doc.save();
  // Simplified: just draw filled circle for score
  doc.fillColor(opts.riskColor).circle(PAGE_W / 2, gaugeY + 30, 24).fill();
  doc.restore();

  setFont(doc, true).fillColor(C.white).fontSize(22)
    .text(`${opts.riskScore}`, gaugeX, gaugeY + 17, { width: gaugeW, align: 'center' });
  setFont(doc, true).fillColor(C.white).fontSize(8)
    .text(`/100  ${opts.riskLevel}`, gaugeX, gaugeY + 40, { width: gaugeW, align: 'center' });
  ...
  setFont(doc, true).fillColor(C.coverAccent).fontSize(9)
    .text('SUJETO DE INVESTIGACIÓN', infoX, infoBoxY + 8, { width: CW - 110 });
  setFont(doc, true).fillColor(C.coverLight).fontSize(13)
    .text(opts.fullName, infoX, infoBoxY + 22, { width: CW - 110 });

  const detailLines: string[] = [];
  if (opts.cedula) detailLines.push(`CC: ${opts.cedula}`);
  if (opts.email) detailLines.push(`✉ ${opts.email}`);
  if (opts.phone) detailLines.push(`☎ ${opts.phone}`);
  if (detailLines.length > 0) {
    setFont(doc, false).fillColor(C.coverSlate).fontSize(8)
      .text(detailLines.join('   |   '), infoX, infoBoxY + 42, { width: CW - 110 });
  }

  setFont(doc, false).fillColor(C.coverSlate).fontSize(7)
    .text(`ID: ${opts.reportId}  |  Fecha: ${opts.today}`, infoX, infoBoxY + 60, { width: CW - 110 });
  ...
  setFont(doc, true).fillColor('#ef4444').fontSize(9)
    .text('DOCUMENTO CONFIDENCIAL — USO RESTRINGIDO', M.left, footerY, { width: CW, align: 'center' });
  setFont(doc, false).fillColor('#64748b').fontSize(6)
    .text('La distribución no autorizada de este documento viola la Ley 1581 de 2012 y la Ley 1273 de 2009', M.left, footerY + 14, { width: CW, align: 'center' });
  setFont(doc, false).fillColor('#475569').fontSize(6)
    .text(`OSINT Data Scanner  |  ${opts.reportId}  |  ${opts.today}`, M.left, footerY + 26, { width: CW, align: 'center' });
}
...
export async function generatePDFReport(data: {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Anonymize all source names before using them
    ...
    if (limitedResults.filter(r => r.severity !== 'info').length === 0 && realResults.length === 0) {
      ...
      for (const severity of displaySeverities) {
        ...
        for (const r of filtered) {
          ...
          // Source (anonymized) and category in gray
          checkPage(doc, 14);
    ...
    // Generic sources statement (NO tool names)
    setFont(doc, true).fillColor(C.navy).fontSize(9)
...
export async function generateSocialPDFReport(data: {
  ...
  return new Promise((resolve, reject) => {
    ...
    // ── PAGE 3-5: RESULTADOS POR PLATAFORMA ──
    doc.addPage();
    ...
    const socialRecs = [
      ...
      '• Eliminar información personal innecesaria de perfiles públicos.',
