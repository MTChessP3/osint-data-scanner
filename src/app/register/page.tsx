  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-4 bg-app-bg">
      ...
      <div className="w-full max-w-md rounded-2xl p-8 shadow-2xl bg-app-surface">
        ...
        <div className="mb-6">
          <h2 className="text-2xl font-bold text-app-text mb-1">Registrarse</h2>
          <p className="text-app-text-muted text-sm">Cree su cuenta para acceder al sistema de inteligencia</p>
        </div>

        <form onSubmit={handleRegister} className="space-y-4">
          {/* Display Name */}
          <div>
            <label className="block text-sm text-app-text-dim mb-1.5">
              Nombre Completo
            </label>
            <div className="relative">
              <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-app-text-muted" />
              <input
                type="text"
                value={displayName}
                onChange={(e) => setDisplayName(e.target.value)}
                className="w-full pl-10 pr-4 py-2.5 rounded-lg text-app-text placeholder:text-app-text-muted focus:outline-none focus:ring-2 transition-all bg-app-bg border border-app-border"
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-12 py-2.5 rounded-lg text-app-text placeholder:text-app-text-muted focus:outline-none focus:ring-2 transition-all bg-app-bg border border-app-border"
              ...
              <div className="mt-2">
                <div className="flex gap-1 mb-1">
                  {[1, 2, 3, 4, 5].map(i => (
                    <div
                      key={i}
                      className={`h-1 flex-1 rounded-full transition-all ${i <= getPasswordStrength() ? '' : 'bg-app-border'}`}
                      style={i <= getPasswordStrength() ? { background: strengthColors[getPasswordStrength()] } : undefined}
                    />
                  ))}
                </div>
            ...
            <div className="mt-2 grid grid-cols-2 gap-1">
              {[
                ...
                <div key={req.label} className={`flex items-center gap-1 text-xs ${req.met ? 'text-green-500' : 'text-app-text-faint'}`}>
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className={`w-full pl-10 pr-12 py-2.5 rounded-lg text-app-text placeholder:text-app-text-muted focus:outline-none focus:ring-2 transition-all bg-app-bg border ${confirmPassword && confirmPassword !== password ? 'border-red-500' : 'border-app-border'}`}
          ...
          <button
            ...
            className={`w-full py-2.5 text-white font-semibold rounded-lg shadow-lg flex items-center justify-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed ${loading ? 'bg-amber-900' : 'bg-amber-500'}`}
          >
            {loading ? (
              <>
        ...
        <div className="text-center pt-4">
          <span className="text-app-text-muted text-sm">¿Ya tiene una cuenta? </span>
          <button
